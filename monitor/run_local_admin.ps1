...
try {
  ...
  # Run in local mode (no WinRM, no credentials)
  $env:MONITOR_MODE = 'local'
  if (-not $env:SAMPLE_INTERVAL_SECONDS) { $env:SAMPLE_INTERVAL_SECONDS = '5' }

  Write-Host 'Starting monitor (local mode) at http://localhost:8000' -ForegroundColor Green
